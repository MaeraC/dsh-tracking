...
function RapportVisitesAdmin({ onReturn, uid }) {
...
const pageRef2 = useRef()
    ...
    const downloadPDF = async () => {
        ...
        try {
            await addDoc(collection(db, "historiqueAdmin"), {
                userId: uid,
                date: new Date(),
                action: `Rapport mensuel des visites téléchargé`,
            })
        } catch (error) {
    ...
    const downloadPDF2 = async () => {
        ...
        generatePDF2(input, `rapport-des-activités-de-${selectedSalon}.pdf`)

        try {
            await addDoc(collection(db, "historiqueAdmin"), {
                userId: uid,
                date: new Date(),
                action: `Rapport des activités de ${selectedSalon} téléchargé`,
            })
        } catch (error) {
            console.error("Erreur lors de l'enregistrement de l'historique : ", error)
        }
    ...
    return (
        <div>
            ...
            <div className="monthly-report-container" style={{width: "100%", display: "flex", justifyContent: "space-around", marginTop: "20px", alignItems: "flex-start" }}>  
                <div className="form-report" style={{width: "30%", margin: "20px", display: "flex", flexDirection: "column"}}>  
                    <form onSubmit={handleGenerateReport} style={{ width: "100%", boxShadow : "2px 2px 15px #cfcfcf", padding: "20px", borderRadius: "20px", marginBottom: "30px"}}>  
                        <h4 style={{fontSize: "18px", marginBottom: "30px", textAlign: "center"}}>Générer le rapport d'un salon spécifique</h4>        
                        <div>
                        ...
                        {isFormSubmitted && selectedSalon && startDate && endDate && (
                         ...
                         {isFormSubmitted && selectedSalon && startDate && endDate && (
                           <div ref={pageRef2} style={{ boxShadow: "2px 2px 15px #cfcfcf", padding: "20px", borderRadius: "20px", width: "100%" }}>
                ...
                <div  className="monthly-report" style={{width: "70%", padding: "30px"}} ref={pageRef} >  
                    <h3 style={{textAlign: "center", fontSize: "18px", marginBottom: "10px"}}>Rapport du mois de {moisEnCours}</h3>
                    <p style={{color: "grey", textAlign: "center", marginBottom: "20px", fontSize: "14px", fontStyle: "italic"}}>Tous les salons visités le mois en cours</p>
                    <div className="report" style={{ width: "100%", display: "flex", flexWrap: "wrap", justifyContent: "start",}}>
                    {monthlyReports.map((report, index) => (
                        <div className="salon-report" key={index} style={{ margin: '20px', boxShadow: "2px 2px 15px #cfcfcf", padding: "20px", borderRadius: "20px" }}>
